...
so building the grid can be computationally expensive if the grid is finer. At the end of all the script runs, you should have output 
ion fractions (saved with extension “.elem”, with elem = carb, nit, oxy, etc.) in the directory for each grid point. Make sure you
can locate these files because they will be parsed in the next step!
